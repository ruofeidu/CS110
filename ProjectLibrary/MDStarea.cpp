...
//FF, GG, HH, II 对1，2，3，4加密进行运算的全局定义
#define XX(X, a, b, c, d, x, s, ac) { \
    (a) += X((b), (c), (d)) + (x) + (UINT4)(ac); \
    (a) = ROTATE_LEFT((a), (s)); \
    (a) += (b); \
    }
//FF, GG, HH, II 对1，2，3，4轮加密进行运算。
...
std::string MDSResult(UINT1 digest[16])
{
    char temp[40] = {0};
    UINT4 i;
...
std::string MDStarea(std::string source)
{
	using namespace std;
	char src[1025];
    ...
    return MDSResult(digest);
}

std::string MD6(std::string source){
	return MDStarea(MDStarea(source));
}